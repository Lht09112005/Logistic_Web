...
export const refreshToken = async (req: Request, res: Response, next?: NextFunction): Promise<void> => {
  try {
    ...
    if (next) {
      ...
      const status = error.statusCode || 500
      sendError(res, error.message || 'Refresh token không hợp lệ hoặc hết hạn', status, status === 500 ? error : undefined)
    }
  }
}
...
export const logout = async (req: AuthRequest, res: Response, next?: NextFunction): Promise<void> => {
  try {
    ...
    if (next) {
      ...
      const status = error.statusCode || 500
      sendError(res, error.message || 'Lỗi đăng xuất', status, status === 500 ? error : undefined)
    }
  }
}
...
export const getDrivers = async (req: Request, res: Response, next?: NextFunction): Promise<void> => {
  try {
    ...
    if (next) {
      ...
      const status = error.statusCode || 500
      sendError(res, error.message || 'Lỗi lấy danh sách tài xế', status, status === 500 ? error : undefined)
    }
  }
}
